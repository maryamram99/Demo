...
if ( apply_filters( 'junotoys_filter_single_post_header', is_singular( 'post' ) || is_singular( 'attachment' ) ) ) {
    $junotoys_post_format = str_replace( 'post-format-', '', get_post_format() );
    $post_meta = in_array( $junotoys_post_format, array( 'video' ) ) ? get_post_meta( get_the_ID(), 'trx_addons_options', true ) : false;
	...
	ob_end_clean();
	$junotoys_with_featured_image = junotoys_is_with_featured_image( $junotoys_post_header /*, array( 'with_gallery' )*/);
	// Post title and meta
	ob_start();
	...
	if ( strpos( $junotoys_post_header, 'post_featured' ) !== false
		...
		<div class="post_header_wrap post_header_wrap_in_header post_header_wrap_style_<?php
            ...
            if ( $junotoys_with_featured_image ) {
                echo ' with_featured_image' . ( false && junotoys_get_theme_option( 'single_parallax' ) == 0 ? ' junotoys-full-height' : '' );
            }
		?>">
			<?php
