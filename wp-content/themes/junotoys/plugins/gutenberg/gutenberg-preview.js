...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			junotoys_create_observer( 'junotoys-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				junotoys_gutenberg_editor_init();
			} );
		} else {
		 	junotoys_gutenberg_editor_init();
		}
	...
	function junotoys_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor.$frame.length ) {
					if ( editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'junotoys_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'junotoys_inited' ) ) {
	...
	// Init on page load
	function junotoys_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = junotoys_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + JUNOTOYS_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + JUNOTOYS_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in JUNOTOYS_STORAGE['override_classes'] ) {
				$obj.addClass( JUNOTOYS_STORAGE['override_classes'][i].replace( '%s', JUNOTOYS_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( junotoys_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && JUNOTOYS_STORAGE['sidebar_position'] != 'hide' ) {
				if ( junotoys_apply_filters( 'junotoys_filter_wrap_gutenberg_editor_with_sidebar', false ) ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'junotoys_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			if ( junotoys_apply_filters( 'junotoys_filter_add_scheme_to_widgets_editor', true ) ) {
				...
				junotoys_remove_observer( 'junotoys-check-editor-styles-wrapper' );
				junotoys_create_observer( 'junotoys-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'junotoys_inited' );
	}
} );
