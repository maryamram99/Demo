...
jQuery( document ).ready(
	function() {
		...
		junotoys_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="junotoys_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.junotoys_post_editor' );
						if ( link.length > 0 ) {
							junotoys_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.junotoys_post_editor', function(e) {
						junotoys_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('junotoys_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function junotoys_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
