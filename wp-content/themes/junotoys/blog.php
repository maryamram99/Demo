...
if ( function_exists( 'junotoys_elementor_is_preview' ) && junotoys_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			junotoys_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	junotoys_new_main_query(
		array(
			'post_type'      => junotoys_get_theme_option( 'post_type' ),
			'category'       => junotoys_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'junotoys_filter_get_template_part', junotoys_blog_archive_get_template() ) );
}
