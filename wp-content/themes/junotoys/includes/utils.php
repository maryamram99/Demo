...
if ( ! function_exists( 'junotoys_array_get_first' ) ) {
	/**
	 ...
	 */
	function junotoys_array_get_first( &$arr, $key = true ) {
		foreach ( $arr as $k => $v ) {
			break;
		}
		return $key ? $k : $v;
...
if ( ! function_exists( 'junotoys_array_get_keys_by_value' ) ) {
	/**
	 ...
	 */
	function junotoys_array_get_keys_by_value( $arr, $value = 1 ) {
		if ( ! is_array( $arr ) ) {
...
if ( ! function_exists( 'junotoys_array_delete_by_value' ) ) {
	/**
	 ...
	 */
	function junotoys_array_delete_by_value( $arr, $value ) {
...
if ( ! function_exists( 'junotoys_array_from_list' ) ) {
	/**
	 ...
	 */
	function junotoys_array_from_list( $arr ) {
		$new = array();
		foreach ( $arr as $v ) {
			$new[ $v ] = $v;
		}
...
if ( ! function_exists( 'junotoys_array_slice' ) ) {
	/**
	 ...
	 */
	function junotoys_array_slice( $arr, $from, $to='' ) {
...
if ( ! function_exists( 'junotoys_array_merge' ) ) {
	 ...
	 * For example:
	 *
	 * junotoys_array_merge( [ 1, 2, 3, 4 ], [ 'one', 'two' ] )  // result: [ 'one', 'two', 3, 4 ]
	 *
	 * junotoys_array_merge( [ 'a' => 'A', 'b' => 'B' ], [ 'one', 'two', 'a' => 'AA' ] )  // result: [ 'a' => 'AA', 'b' => 'B', 0 => 'one', 1 => 'two' ]
	 *
...
if ( ! function_exists( 'junotoys_array_insert_after' ) ) {
	/**
	...
	function junotoys_array_insert_after( &$haystack, $needle, $stuff ) {
		...
		$haystack = is_int( $needle )
						...
						: junotoys_array_merge( array_slice( $haystack, 0, $i, true ), $new_array, array_slice( $haystack, $i, null, true ) );

		return $i;
	}
}

if ( ! function_exists( 'junotoys_array_insert_before' ) ) {
	/**
	 * Inserts any number of scalars or arrays at the point in the haystack
...
if ( ! function_exists( 'junotoys_array_get_by_key' ) ) {
	/**
	 ...
	 */
	function junotoys_array_get_by_key( &$arr, $key, $value, $key2 = '', $default = '' ) {
		$rez = $default;
		foreach ( $arr as $v ) {
...
if ( ! function_exists( 'junotoys_array_str_replace' ) ) {
	/**
	 ...
	 */
	function junotoys_array_str_replace( $from, $to, $key, $arr ) {
		foreach ( $arr as $k => $v ) {
			if ( is_array( $v ) ) {
				$arr[ $k ] = junotoys_array_str_replace( $from, $to, $key, $v );
			} else if ( $k == $key ) {
...
if ( ! function_exists( 'junotoys_generate_id' ) ) {
	/**
	 ...
	 */
	function junotoys_generate_id( $prefix = '' ) {
		return $prefix . str_replace( '.', '', mt_rand() );
	}
}

if ( ! function_exists( 'junotoys_get_tag' ) ) {
	/**
	 ...
	 */
	function junotoys_get_tag( $text, $tag_start, $tag_end = '' ) {
		$val       = '';
...
if ( ! function_exists( 'junotoys_get_css_position_from_values' ) ) {
	...
	function junotoys_get_css_position_from_values( $top = '', $right = '', $bottom = '', $left = '', $width = '', $height = '' ) {
		...
		foreach ( $top as $k => $v ) {
			...
			if ( '' != $v ) {
				$output .= ( 'width' == $k ? 'width' : ( 'height' == $k ? 'height' : 'margin-' . esc_attr( $k ) ) ) . ':' . esc_attr( junotoys_prepare_css_value( $v ) ) . ( '!' == $imp ? ' !important' : '' ) . ';';
			}
		}
		return $output;
	}
...
if ( ! function_exists( 'junotoys_get_column_class' ) ) {
	/**
	 ...
	 */
	function junotoys_get_column_class( $num, $all, $all_tablet = '', $all_mobile = '' ) {
...
if ( ! function_exists( 'junotoys_get_svg_from_file' ) ) {
	...
	function junotoys_get_svg_from_file( $svg ) {
		if ( junotoys_is_url( $svg ) && ! junotoys_is_external_url( $svg ) ) {
			$svg_path = junotoys_url_to_local_path( $svg );
			if ( ! empty( $svg_path ) ) {
				$svg = $svg_path;
			}
		}
...
if ( ! function_exists( 'junotoys_stripslashes' ) ) {
	/**
	 ...
	 */
	function junotoys_stripslashes( $val ) {
		static $magic = 0;
		if ( 0 === $magic ) {
...
if ( ! function_exists( 'junotoys_get_value_gpc' ) ) {
	 ...
	 */
	function junotoys_get_value_gpc( $name, $defa = '' ) {
		if ( isset( $_GET[ $name ] ) ) {
			$rez = wp_unslash( $_GET[ $name ] );
		} elseif ( isset( $_POST[ $name ] ) ) {
			$rez = wp_unslash( $_POST[ $name ] );
...
if ( ! function_exists( 'junotoys_get_value_gps' ) ) {
	 ...
	 */
	function junotoys_get_value_gps( $name, $defa = '' ) {
		global $wp_session;
		if ( isset( $_GET[ $name ] ) ) {
			$rez = wp_unslash( $_GET[ $name ] );
		} elseif ( isset( $_POST[ $name ] ) ) {
...
if ( ! function_exists( 'junotoys_get_session_value' ) ) {
	/**
	 ...
	 */
	function junotoys_get_session_value( $name, $defa = '' ) {
		global $wp_session;
...
if ( ! function_exists( 'junotoys_set_session_value' ) ) {
	 ...
	 */
	function junotoys_set_session_value( $name, $value ) {
		global $wp_session;
...
if ( ! function_exists( 'junotoys_del_session_value' ) ) {
	 ...
	 */
	function junotoys_del_session_value( $name ) {
		global $wp_session;
		unset( $wp_session[ $name ] );
	}
}

if ( ! function_exists( 'junotoys_get_cookie' ) ) {
	/**
	 ...
	 */
	function junotoys_get_cookie( $name, $defa = '' ) {
		return junotoys_stripslashes( isset( $_COOKIE[ $name ] ) ? $_COOKIE[ $name ] : $defa );
	}
}

// Set a cookie - wrapper for setcookie using WP constants.
if ( ! function_exists( 'junotoys_set_cookie' ) ) {
	/**
	 ...
	 */
	function junotoys_set_cookie( $name, $value, $expire = 0, $secure = false, $httponly = false  ) {
		if ( ! headers_sent() ) {
			if ( defined( 'PHP_VERSION_ID' ) && PHP_VERSION_ID >= 70300 ) {
				$rez = setcookie(
							$name,
							$value,
							apply_filters( 'junotoys_filter_cookie_options', array(
								'expires'  => $expire,
								'path'     => defined( 'COOKIEPATH' ) && ! empty( COOKIEPATH ) ? COOKIEPATH : '/',
								'domain'   => defined( 'COOKIE_DOMAIN' ) && ! empty( COOKIE_DOMAIN ) ? COOKIE_DOMAIN : '',	//junotoys_get_domain_from_url( home_url() ),
								'secure'   => $secure,
								'httponly' => $httponly,
				...
				$rez = setcookie(
							...
							defined( 'COOKIE_DOMAIN' ) && ! empty( COOKIE_DOMAIN ) ? COOKIE_DOMAIN : '',	//junotoys_get_domain_from_url( home_url() ),
							$secure,
							$httponly
						);
			}
			...
			trigger_error( sprintf( __( '%1$s cookie cannot be set - headers already sent by %2$s on line %3$s', 'junotoys' ), $name, $file, $line ), E_USER_NOTICE ); // @codingStandardsIgnoreLine
		}
	}
}
...
if ( ! function_exists( 'junotoys_rgba2hex' ) ) {
	/**
	 ...
	 */
	function junotoys_rgba2hex( $color, $alpha = false ) {
...
if ( ! function_exists( 'junotoys_hex2hsb' ) ) {
	 ...
	 */
	function junotoys_hex2hsb( $hex, $h = 0, $s = 0, $b = 0 ) {
		$hsb      = junotoys_rgb2hsb( junotoys_hex2rgb( $hex ) );
...
if ( ! function_exists( 'junotoys_format_price' ) ) {
	/**
	 ...
	 */
	function junotoys_format_price( $price ) {
...
if ( ! function_exists( 'junotoys_num2size' ) ) {
	/**
	 ...
	 */
	function junotoys_num2size( $num ) {
		return $num > 1000 ? round( $num/1000, 0 ) . 'K' : $num;
	}
}

if ( ! function_exists( 'junotoys_size2num' ) ) {
	/**
	 ...
	 */
	function junotoys_size2num( $size, $full = false ) {
...
//
if ( ! function_exists( 'junotoys_parse_num' ) ) {
	/**
	 ...
	 */
	function junotoys_parse_num( $str ) {
...
if ( ! function_exists( 'junotoys_prepare_macros' ) ) {
	/**
	 ...
	 * @return string      A processed string.
	 */
	function junotoys_prepare_macros( $str ) {
...
if ( ! function_exists( 'junotoys_remove_macros' ) ) {
	/**
	 ...
	 * @return string      A processed string.
	 */
	function junotoys_remove_macros( $str ) {
		return empty( $str ) ? '' : str_replace(
										array( '{{', '}}', '((', '))', '||' ),
										array( '', '', '', '', ' ' ),
										$str
...
if ( ! function_exists( 'junotoys_is_on' ) ) {
	/**
...
if ( ! function_exists( 'junotoys_is_off' ) ) {
	/**
...
if ( ! function_exists( 'junotoys_is_inherit' ) ) {
	/**
	 ...
	 */
	function junotoys_is_inherit( $prm ) {
...
if ( ! function_exists( 'junotoys_strshort' ) ) {
	 ...
	 */
	function junotoys_strshort( $str, $maxlength, $add = '&hellip;' ) {
		if ( 0 >= $maxlength ) {
			return '';
		}
		$str = junotoys_strip_tags( $str );
...
if ( ! function_exists( 'junotoys_strwords' ) ) {
	/**
	 ...
	 * @return string            A truncated string.
	 */
	function junotoys_strwords( $str, $maxlength, $add = '&hellip;' ) {
		if ( $maxlength <= 0 ) {
			return '';
		}
		$words = explode( ' ', $str );
...
if ( ! function_exists( 'junotoys_strip_tags' ) ) {
	/**
	 ...
	 */
	function junotoys_strip_tags( $str ) {
...
if ( ! function_exists( 'junotoys_excerpt' ) ) {
	/**
	 ...
	 */
	function junotoys_excerpt( $str, $maxlength, $add = '&hellip;' ) {
		if ( $maxlength <= 0 ) {
			return '';
		}
		return junotoys_strwords( junotoys_strip_tags( $str ), $maxlength, $add );
	}
}

if ( ! function_exists( 'junotoys_unserialize_recover' ) ) {
	/**
	 ...
	 * @return string      A processed string.
	 */
	function junotoys_unserialize_recover( $str ) {
		...
		if ( ! empty( $str ) && is_serialized( $str ) ) {
			...
			if ( true || ! preg_match( '/O:[0-9]+:"([^"]*)":[0-9]+:{/', $str, $matches ) || empty( $matches[1] ) || class_exists( $matches[1] ) ) {
				try {
					...
					if ( false === $data ) {
						$data = @unserialize( junotoys_unserialize_recover( $str ) );
					}
					// Attempt 4: try unserialize original string without CR symbol '\r' with modified character counters
					if ( false === $data ) {
						$data = @unserialize( junotoys_unserialize_recover( $str2 ) );
					}
				} catch ( Exception $e ) {
...
if ( ! function_exists( 'junotoys_str_replace' ) ) {
	/**
	 ...
	 */
	function junotoys_str_replace( $from, $to, $str ) {
		if ( is_array( $str ) ) {
			foreach ( $str as $k => $v ) {
				$str[ $k ] = junotoys_str_replace( $from, $to, $v );
			}
			...
			if ( is_serialized( $str ) ) {
				$str = serialize( junotoys_str_replace( $from, $to, junotoys_unserialize( $str ) ) );
			} else {
...
if ( ! function_exists( 'junotoys_str_replace_once' ) ) {
	/**
	 ...
	 * @return string             A processed string.
	 */
	function junotoys_str_replace_once( $from, $to, $str ) {
		$rez = '';
		if ( ! is_array( $from ) ) {
			$from = array( $from );
		}
...
if ( ! function_exists( 'junotoys_tags_count' ) ) {
	/**
	 ...
	 */
	function junotoys_tags_count( $str, $tag ) {
		$cnt = 0;
...
if ( ! function_exists( 'junotoys_getimagesize' ) ) {
	 ...
	 */
	function junotoys_getimagesize( $url, $echo = false ) {
		$img_size = false;
		if ( is_string( $url ) ) {
			$img_path = junotoys_is_url( $url ) ? junotoys_url_to_local_path( $url ) : $url;
...
if ( ! function_exists( 'junotoys_clear_thumb_size' ) ) {
	/**
	 ...
	 */
	function junotoys_clear_thumb_size( $url, $remove_protocol = true ) {
		if ( empty( $url ) ) return '';
		$pi = pathinfo( $url );
		if ( $remove_protocol ) {
			$pi['dirname'] = junotoys_remove_protocol_from_url( $pi['dirname'], false );
		}
...
if ( ! function_exists( 'junotoys_add_thumb_size' ) ) {
	/**
	 ...
	 */
	function junotoys_add_thumb_size( $url, $thumb_size, $check_exists = true ) {
		...
		$new_url = junotoys_remove_protocol_from_url( ( ! empty( $pi['dirname'] ) ? $pi['dirname'] . '/' : '' ) . $pi['filename'] . ( ! empty( $pi['extension'] ) ? '.' . $pi['extension'] : '' ) );

		// Check exists
		if ( $check_exists ) {
			$uploads_info = wp_upload_dir();
			$uploads_url = junotoys_remove_protocol_from_url( $uploads_info['baseurl'] );
			...
			if ( strpos( $new_url, $uploads_url ) !== false ) {
				if ( ! file_exists( str_replace( $uploads_url, $uploads_dir, $new_url ) ) ) {
					$new_url = junotoys_remove_protocol_from_url( $url );
				}
			} else {
				$new_url = junotoys_remove_protocol_from_url( $url );
			}
...
if ( ! function_exists( 'junotoys_get_thumb_size' ) ) {
	 ...
	 */
	function junotoys_get_thumb_size( $ts ) {
		$thumb_sizes = junotoys_storage_get( 'theme_thumbs' );
		if ( ! empty( $thumb_sizes[ "junotoys-thumb-{$ts}" ] ) ) {
			$ts = 'junotoys-thumb-' . $ts;
		}
		$retina = junotoys_get_retina_multiplier() > 1 ? '-@retina' : '';
		$is_external = apply_filters(
							'junotoys_filter_is_external_thumb_size',
							in_array( $ts,
										apply_filters(
											'junotoys_filter_external_thumb_sizes',
		...
		$is_internal = strpos( $ts, 'junotoys-thumb-' ) === 0 || strpos( $ts, 'trx_addons-thumb-' ) === 0;
		return apply_filters(
					'junotoys_filter_get_thumb_size',
					( $is_external || $is_internal ? '' : 'junotoys-thumb-' )
					. $ts
					. ( $is_internal ? $retina : '' )
				);
	}
...
if ( ! function_exists( 'junotoys_get_closest_thumb_size' ) ) {
	 ...
	 */
	function junotoys_get_closest_thumb_size( $old_size, $dim, $prefix = '' ) {
...
if ( ! function_exists( 'junotoys_get_attachment_url' ) ) {
	/**
	 ...
	 * @return string
	 */
	function junotoys_get_attachment_url( $image_id, $size = 'full' ) {
		...
		if ( is_numeric( $image_id ) && (int) $image_id > 0 ) {
			...
			$image_id = junotoys_add_thumb_size( $image_id, $size );
		}
		return $image_id;
	}
}
...
if ( ! function_exists( 'junotoys_get_post_image' ) ) {
	 ...
	 */
	function junotoys_get_post_image( $post_text = '', $src = true ) {
		global $post;
		$img = '';
		if ( empty( $post_text ) ) {
...
if ( ! function_exists( 'junotoys_get_post_audio' ) ) {
	 ...
	 * @return string           Founded URL (or a whole tag) or an empty string if found nothing.
	 */
	function junotoys_get_post_audio( $post_text = '', $src = true ) {
		global $post;
		$img = junotoys_get_post_audio_list_first( ! $src );
		if ( is_array( $img ) ) {
			if ( ! empty( $img['url'] ) ) {
				$img = $img['url'];
			} else if ( ! empty( $img['embed'] ) ) {
				$img = junotoys_get_post_iframe( $img['embed'], true );
			} else {
				$img = '';
			}
		}
		if ( empty( $img ) ) {
			...
			if ( $src ) {
				...
				$img = junotoys_get_tag( $post_text, '<audio', '</audio>' );
				if ( empty( $img ) ) {
					$img = do_shortcode( junotoys_get_tag( $post_text, '[audio', '[/audio]' ) );
				}
				if ( empty( $img ) ) {
					$img = junotoys_get_tag_attrib( $post_text, '[trx_widget_audio]', 'url' );
...
if ( ! function_exists( 'junotoys_get_post_audio_list' ) ) {
	/**
	 ...
	 *                     ]
	 */
	function junotoys_get_post_audio_list( $args=array() ) {
		return function_exists( 'trx_addons_get_post_audio_list' ) ? trx_addons_get_post_audio_list( $args ) : array();
	}
}

if ( ! function_exists( 'junotoys_get_post_audio_list_first' ) ) {
	/**
	 ...
	 *                     or the URL only
	 */
	function junotoys_get_post_audio_list_first( $src = false, $args = array() ) {
...
if ( ! function_exists( 'junotoys_get_post_video' ) ) {
	 ...
	 * @return string           Founded URL (or a whole tag) or an empty string if found nothing.
	 */
	function junotoys_get_post_video( $post_text = '', $src = true ) {
		global $post;
		$img = junotoys_get_post_video_list_first( ! $src );
		if ( is_array( $img ) ) {
			if ( ! empty( $img['video_url'] ) ) {
				$img = $img['video_url'];
			} else if ( ! empty( $img['video_embed'] ) ) {
				$img = junotoys_get_post_iframe( $img['video_embed'], true );
			} else {
				$img = '';
			}
		}
		...
		return apply_filters( 'junotoys_filter_get_post_video', $img );
	}
}

if ( ! function_exists( 'junotoys_get_post_video_list' ) ) {
	/**
	 ...
	 *                     ]
	 */
	function junotoys_get_post_video_list( $args=array() ) {
		return function_exists( 'trx_addons_get_post_video_list' ) ? trx_addons_get_post_video_list( $args ) : array();
	}
}

if ( ! function_exists( 'junotoys_get_post_video_list_first' ) ) {
	/**
	 ...
	 *                     or the URL only
	 */
	function junotoys_get_post_video_list_first( $src = false, $args = array() ) {
		return function_exists( 'trx_addons_get_post_video_list_first' ) ? trx_addons_get_post_video_list_first( $src, $args ) : '';
	}
}

if ( ! function_exists( 'junotoys_get_post_iframe' ) ) {
	/**
	 ...
	 * @return string           Founded URL (or a whole tag) or an empty string if found nothing.
	 */
	function junotoys_get_post_iframe( $post_text = '', $src = true ) {
		global $post;
		$img = '';
		$tag = junotoys_get_embed_video_tag_name();
		if ( empty( $post_text ) ) {
			$post_text = $post->post_content;
		}
		if ( $src ) {
			...
			$img = junotoys_get_tag( $post_text, '<' . esc_html( $tag ), '</' . esc_html( $tag ) . '>' );
		}
		return apply_filters( 'junotoys_filter_get_post_iframe', $img );
	}
}

if ( ! function_exists( 'junotoys_get_embed_video' ) ) {
	/**
	 ...
	 */
	function junotoys_get_embed_video( $video, $use_wp_embed = false, $args = array() ) {
		global $wp_embed;
		if ( $use_wp_embed && is_object( $wp_embed ) ) {
			$embed_video = do_shortcode( $wp_embed->run_shortcode( '[embed]' . trim( $video ) . '[/embed]' ) );
		} else if ( junotoys_is_from_uploads( $video ) ) {
			$embed_video  = '<video'
								. ' src="' . esc_url( $video ) . '"'
			...
			if ( strpos( $video, 'youtu.be/' ) !== false || strpos( $video, 'youtube.com/' ) !== false ) {
				...
				$video = junotoys_add_to_url( $video, junotoys_get_embed_video_parameters() );
			}
			...
			$ratio  = apply_filters( 'junotoys_filter_video_ratio', $is_tiktok ? '9:21' : '16:9' );
			$parts  = explode( ':', $ratio );
			$width  = $is_tiktok ? apply_filters( 'junotoys_filter_video_width', 325, 'tiktok' ) : junotoys_get_content_width();
			$height = round( $width / $parts[0] * $parts[1] );
			$dim    = apply_filters( 'junotoys_filter_video_dimensions', array(
									'width' => $width,
									'height' => $height
									), $ratio );
...
if ( ! function_exists( 'junotoys_get_embed_video_tag_name' ) ) {
	/**
	 * Return a name of the tag-container to embed video.
	 *
	 * @return string  A tag name to embed video.
	 */
	function junotoys_get_embed_video_tag_name() {
...
if ( ! function_exists( 'junotoys_make_video_autoplay' ) ) {
	/**
	 ...
	 */
	function junotoys_make_video_autoplay( $video, $muted = false ) {
					...
					if ( ! empty( $matches[2] ) ) {
						...
						$matches[2] = junotoys_add_to_url(
							...
							array_merge(
								...
								$muted
									? array_merge(
										junotoys_get_embed_video_parameters(),
										array(
											'muted' => 1,
											'controls' => 0,
											'background' => 1,
...
if ( ! function_exists( 'junotoys_is_from_uploads' ) ) {
	/**
	 ...
	 */
	function junotoys_is_from_uploads( $url ) {
		$url          = junotoys_remove_protocol_from_url( $url );
		$parts        = explode( '?', $url );
		$url          = $parts[0];
		$uploads_info = wp_upload_dir();
		$uploads_url  = junotoys_remove_protocol_from_url( $uploads_info['baseurl'] );
	...
	function junotoys_fpc( $file, $data, $flag = 0 ) {
		if ( ! empty( $file ) ) {
			if ( function_exists( 'trx_addons_fpc' ) ) {
				...
				$file = junotoys_prepare_path( $file );
...
if ( ! function_exists( 'junotoys_mkdir' ) ) {
	/**
...
if ( ! function_exists( 'junotoys_unlink' ) ) {
	/**
	 ...
	 */
	function junotoys_unlink( $path, $recursive = true, $type = 'd' ) {
		if ( ! empty( $path ) ) {
...
if ( ! function_exists( 'junotoys_copy' ) ) {
	/**
	 ...
	 */
	function junotoys_copy( $src, $dst ) {
		if ( ! empty( $src ) && ! empty( $dst ) ) {
			if ( function_exists( 'trx_addons_copy' ) ) {
				...
				if ( is_dir( $src ) ) {
					...
					foreach ( $files as $file ) {
						if ( $file != "." && $file != ".." ) {
							junotoys_copy( "$src/$file", "$dst/$file" );
						}
					}
					return true;
...
if ( ! function_exists( 'junotoys_unzip_file' ) ) {
	/**
	 ...
	 * @return bool         Return true if no error occurs.
	 */
	function junotoys_unzip_file( $zip, $dest ) {
...
if ( ! function_exists( 'junotoys_retrieve_json' ) ) {
	/**
	 ...
	 */
	function junotoys_retrieve_json( $url ) {
...
if ( ! function_exists( 'junotoys_esc' ) ) {
	 ...
	 */
	function junotoys_esc( $name ) {
		return str_replace( array( '\\', '~', '$', ':', ';', '+', '>', '<', '|', '"', "'", '`', "\xFF", "\x0A", "\x0D", '*', '?', '^' ), '/', trim( $name ) );
	}
}

if ( ! function_exists( 'junotoys_prepare_path' ) ) {
	/**
	 ...
	 */
	function junotoys_prepare_path( $path ) {
		return str_replace( array( '\\', '/' ), defined( 'DIRECTORY_SEPARATOR' ) ? DIRECTORY_SEPARATOR : '/', $path );
	}
}

if ( ! function_exists( 'junotoys_check_min_file' ) ) {
	/**
	 ...
	 */
	function junotoys_check_min_file( $file, $dir = '' ) {
		if ( empty( $dir ) ) {
			$dir = dirname( $file );
		}
		if ( substr( $file, -3 ) == '.js' ) {
			if ( substr( $file, -7 ) != '.min.js' && junotoys_is_off( junotoys_get_theme_option( 'debug_mode' ) ) ) {
			...
			if ( substr( $file, -8 ) != '.min.css' && junotoys_is_off( junotoys_get_theme_option( 'debug_mode' ) ) ) {
				$dir      = trailingslashit( $dir );
...
if ( ! function_exists( 'junotoys_get_file_dir' ) ) {
	...
	function junotoys_get_file_dir( $file, $return_url = false ) {
		...
		if ( function_exists( 'get_theme_file_path' ) && ! JUNOTOYS_ALLOW_SKINS && ! junotoys_get_theme_setting( 'check_min_version', false ) ) {
		...
		// Otherwise (on WordPress older then 4.7.0) or theme use .min versions of .js and .css or theme use skins
			...
			$dir       = '';
			$theme_dir = apply_filters( 'junotoys_filter_get_theme_file_dir', '', $file, $return_url );
			if ( '' != $theme_dir ) {
				$dir = $theme_dir;
			} elseif ( JUNOTOYS_CHILD_DIR != JUNOTOYS_THEME_DIR && file_exists( JUNOTOYS_CHILD_DIR . ( $file ) ) ) {
				$dir = ( $return_url ? JUNOTOYS_CHILD_URL : JUNOTOYS_CHILD_DIR ) . junotoys_check_min_file( $file, JUNOTOYS_CHILD_DIR );
			} elseif ( file_exists( JUNOTOYS_THEME_DIR . ( $file ) ) ) {
				$dir = ( $return_url ? JUNOTOYS_THEME_URL : JUNOTOYS_THEME_DIR ) . junotoys_check_min_file( $file, JUNOTOYS_THEME_DIR );
			}
		}
		return $dir;
	}
...
if ( ! function_exists( 'junotoys_get_file_name' ) ) {
	/**
	 ...
	 */
	function junotoys_get_file_name( $file, $without_ext = true ) {
		$parts = pathinfo( $file );
...
if ( ! function_exists( 'junotoys_get_folder_dir' ) ) {
	...
	function junotoys_get_folder_dir( $folder, $return_url = false ) {
		...
		$dir       = '';
		$theme_dir = apply_filters( 'junotoys_filter_get_theme_folder_dir', '', $folder, $return_url );
		if ( '' != $theme_dir ) {
			$dir = $theme_dir;
		} elseif ( JUNOTOYS_CHILD_DIR != JUNOTOYS_THEME_DIR && is_dir( JUNOTOYS_CHILD_DIR . ( $folder ) ) ) {
			$dir = ( $return_url ? JUNOTOYS_CHILD_URL : JUNOTOYS_CHILD_DIR ) . ( $folder );
		} elseif ( is_dir( JUNOTOYS_THEME_DIR . ( $folder ) ) ) {
			$dir = ( $return_url ? JUNOTOYS_THEME_URL : JUNOTOYS_THEME_DIR ) . ( $folder );
		}
		return apply_filters( 'junotoys_filter_get_folder_dir', $dir, $folder, $return_url );
	}
}
...
if ( ! function_exists( 'junotoys_merge_js' ) ) {
	/**
	 ...
	 */
	function junotoys_merge_js( $to, $list ) {
		$s = '';
		foreach ( $list as $f ) {
			$s .= junotoys_fgc( junotoys_get_file_dir( $f ) );
		}
		if ( '' != $s ) {
			$file_dir = junotoys_get_file_dir( $to );
			if ( empty( $file_dir ) && strpos( $to, '-full.js' ) !== false ) {
				$file_dir = junotoys_get_file_dir( str_replace( '-full.js', '.js', $to ) );
				if ( ! empty( $file_dir ) ) {
					$file_dir = str_replace( '.js', '-full.js', $file_dir );
				}
			}
			junotoys_fpc( $file_dir,
				'/* '
				. strip_tags( __( "ATTENTION! This file was generated automatically! Don't change it!!!", 'junotoys' ) )
				. "\n----------------------------------------------------------------------- */\n"
				. apply_filters( 'junotoys_filter_js_output', apply_filters( 'junotoys_filter_prepare_js', $s, true ), $to )
			);
		}
	}
}

if ( ! function_exists( 'junotoys_merge_css' ) ) {
	/**
	 ...
	 */
	function junotoys_merge_css( $to, $list, $need_responsive = false ) {
		if ( $need_responsive ) {
			$responsive = apply_filters( 'junotoys_filter_responsive_sizes', junotoys_storage_get( 'responsive' ) );
		}
		$sizes  = array();
		$output = '';
		foreach ( $list as $f ) {
			$fdir = junotoys_get_file_dir( $f );
			if ( '' != $fdir ) {
				$css = junotoys_fgc( $fdir );
				if ( $need_responsive ) {
					$pos = 0;
					while( false !== $pos ) {
						...
						if ( false !== $pos ) {
							...
							$sizes[ $media ] .= "\n\n" . apply_filters( 'junotoys_filter_merge_css', substr( $css, $pos_lbrace + 1, $pos_rbrace - $pos_lbrace - 1 ) );
							$css = substr( $css, $pos_rbrace + 1);
						}
					}
				} else {
					$output .= "\n\n" . apply_filters( 'junotoys_filter_merge_css', $css );
				}
			}
		}
		if ( $need_responsive ) {
			foreach ( $responsive as $k => $v ) {
				...
				if ( ! empty( $sizes[ $media ] ) ) {
					$output .= "\n\n"
							...
							. strip_tags( sprintf( __( '/* SASS Suffix: --%s */', 'junotoys' ), $k ) )
							. "\n"
							. "@media {$media} {\n"
								. $sizes[ $media ]
							. "\n}\n";
			...
			if ( count( $sizes ) > 0 ) {
				$output .= "\n\n"
						. strip_tags( __( '/* Unknown Suffixes: */', 'junotoys' ) );
		...
		if ( '' != $output ) {
			...
			if ( empty( $file_dir ) && strpos( $to, '-full.css' ) !== false ) {
				$file_dir = junotoys_get_file_dir( str_replace( '-full.css', '.css', $to ) );
				if ( ! empty( $file_dir ) ) {
			...
			junotoys_fpc( $file_dir,
				...
				. strip_tags( __("ATTENTION! This file was generated automatically! Don't change it!!!", 'junotoys') ) 
				. "\n----------------------------------------------------------------------- */\n"
				. apply_filters( 'junotoys_filter_css_output', apply_filters( 'junotoys_filter_prepare_css', $output, true ), $to )
			);
		}
	}
}

if ( ! function_exists( 'junotoys_filter_merge_css' ) ) {
	add_filter( 'junotoys_filter_merge_css', 'junotoys_filter_merge_css' );
	/**
	 ...
	 */
	function junotoys_filter_merge_css( $css ) {
		return str_replace( '../../../../', '../../../', $css);
	}
}

if ( ! function_exists( 'junotoys_merge_sass' ) ) {
	/**
	 ...
	 */
	function junotoys_merge_sass( $to, $list, $need_responsive = false, $root = '../' ) {
		if ( $need_responsive ) {
			$responsive = apply_filters( 'junotoys_filter_responsive_sizes', junotoys_storage_get( 'responsive' ) );
		}
		$sass                = array(
		...
		foreach ( $list as $f ) {
			...
			if ( $add ) {
				$sass['import'] .= apply_filters( 'junotoys_filter_sass_import', "{$sass_import} \"{$root}{$f}\";\n", $f );
				$save            = true;
			}
		}
		if ( $save ) {
			...
			junotoys_fpc( junotoys_get_file_dir( $to ), apply_filters( 'junotoys_filter_sass_output', $output, $to ) );
		}
	}
}
...
if ( ! function_exists( 'junotoys_add_protocol' ) ) {
	/**
	 ...
	 */
	function junotoys_add_protocol( $url ) {
		return substr( $url, 0, 2 ) == '//' ? junotoys_get_protocol() . ':' . $url : $url;
	}
}

if ( ! function_exists( 'junotoys_remove_protocol_from_url' ) ) {
	/**
	 ...
	 * @return string         A processed string.
	 */
	function junotoys_remove_protocol_from_url( $url, $complete = true ) {
...
if ( ! function_exists( 'junotoys_add_to_url' ) ) {
	...
	function junotoys_add_to_url( $url, $prm ) {
		if ( is_array( $prm ) && count( $prm ) > 0 ) {
			...
			$params = junotoys_array_merge( $params, $prm );
			$url = $parts[0];
			$separator = '?';
...
if ( ! function_exists( 'junotoys_is_url' ) ) {
	/**
	 ...
	 */
	function junotoys_is_url( $url ) {
		return is_string( $url ) && ( strpos( $url, '//' ) === 0 || strpos( $url, '://' ) !== false );
	}
}

if ( ! function_exists( 'junotoys_is_external_url' ) ) {
	/**
	 ...
	 */
	function junotoys_is_external_url( $url ) {
		...
		return junotoys_is_url( $url ) && strpos( $url, junotoys_remove_protocol_from_url( $parts[0], true ) ) === false;
	}
}

if ( ! function_exists( 'junotoys_url_to_local_path' ) ) {
	/**
	 ...
	 */
	function junotoys_url_to_local_path( $url ) {
		$path = '';
		...
		$locations = array(
			'uploads' => array(
				...
				'url' => junotoys_remove_protocol_from_url( $upload_info['baseurl'] )
				),
			'child' => array(
				'dir' => JUNOTOYS_CHILD_DIR,
				'url' => junotoys_remove_protocol_from_url( JUNOTOYS_CHILD_URL )
				),
			'theme' => array(
				'dir' => JUNOTOYS_THEME_DIR,
				'url' => junotoys_remove_protocol_from_url( JUNOTOYS_THEME_URL )
				)
			);
