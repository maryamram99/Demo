...
if ( ! function_exists( 'junotoys_storage_unset' ) ) {
	/**
	 ...
	 */
	function junotoys_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $JUNOTOYS_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			if ( isset( $JUNOTOYS_STORAGE[ $var_name ][ $key ][ $key2 ] ) ) {
				unset( $JUNOTOYS_STORAGE[ $var_name ][ $key ][ $key2 ] );
			}
		} elseif ( '' !== $key ) {
			if ( isset( $JUNOTOYS_STORAGE[ $var_name ][ $key ] ) ) {
				unset( $JUNOTOYS_STORAGE[ $var_name ][ $key ] );
			}
		} else {
			if ( isset( $JUNOTOYS_STORAGE[ $var_name ] ) ) {
				unset( $JUNOTOYS_STORAGE[ $var_name ] );
			}
		}
	}
}
...
if ( ! function_exists( 'junotoys_storage_inc' ) ) {
	...
	function junotoys_storage_inc( $var_name, $value = 1 ) {
		global $JUNOTOYS_STORAGE;
		if ( empty( $JUNOTOYS_STORAGE[ $var_name ] ) ) {
			$JUNOTOYS_STORAGE[ $var_name ] = 0;
		}
		$JUNOTOYS_STORAGE[ $var_name ] += $value;
	}
}
...
if ( ! function_exists( 'junotoys_storage_get_array' ) ) {
	...
	function junotoys_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $JUNOTOYS_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $JUNOTOYS_STORAGE[ $var_name ][ $key ] ) ? $JUNOTOYS_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $JUNOTOYS_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $JUNOTOYS_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}
...
if ( ! function_exists( 'junotoys_storage_set_array' ) ) {
	...
	function junotoys_storage_set_array( $var_name, $key, $value ) {
		global $JUNOTOYS_STORAGE;
		if ( ! isset( $JUNOTOYS_STORAGE[ $var_name ] ) ) {
			$JUNOTOYS_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $JUNOTOYS_STORAGE[ $var_name ] ) ) {
			if ( '' === $key ) {
				$JUNOTOYS_STORAGE[ $var_name ][] = $value;
			} else {
...
if ( ! function_exists( 'junotoys_storage_set_array2' ) ) {
	...
	function junotoys_storage_set_array2( $var_name, $key, $key2, $value ) {
		...
		if ( ! isset( $JUNOTOYS_STORAGE[ $var_name ][ $key ] ) ) {
			$JUNOTOYS_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( is_array( $JUNOTOYS_STORAGE[ $var_name ][ $key ] ) ) {
			if ( '' === $key2 ) {
				$JUNOTOYS_STORAGE[ $var_name ][ $key ][] = $value;
			} else {
...
if ( ! function_exists( 'junotoys_storage_merge_array' ) ) {
	 ...
	 */
	function junotoys_storage_merge_array( $var_name, $key, $value ) {
		global $JUNOTOYS_STORAGE;
		if ( ! isset( $JUNOTOYS_STORAGE[ $var_name ] ) ) {
			$JUNOTOYS_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$JUNOTOYS_STORAGE[ $var_name ] = array_merge( $JUNOTOYS_STORAGE[ $var_name ], $value );
		} else {
			$JUNOTOYS_STORAGE[ $var_name ][ $key ] = array_merge( $JUNOTOYS_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}
...
if ( ! function_exists( 'junotoys_storage_inc_array' ) ) {
	...
	function junotoys_storage_inc_array( $var_name, $key, $value = 1 ) {
		...
		if ( empty( $JUNOTOYS_STORAGE[ $var_name ][ $key ] ) ) {
			$JUNOTOYS_STORAGE[ $var_name ][ $key ] = 0;
		}
		$JUNOTOYS_STORAGE[ $var_name ][ $key ] += $value;
	}
}
