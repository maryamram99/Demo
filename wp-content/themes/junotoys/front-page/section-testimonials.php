<div class="front_page_section front_page_section_testimonials<?php
	$junotoys_scheme = junotoys_get_theme_option( 'front_page_testimonials_scheme' );
	if ( ! empty( $junotoys_scheme ) && ! junotoys_is_inherit( $junotoys_scheme ) ) {
		echo ' scheme_' . esc_attr( $junotoys_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( junotoys_get_theme_option( 'front_page_testimonials_paddings' ) );
	if ( junotoys_get_theme_option( 'front_page_testimonials_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$junotoys_css      = '';
		$junotoys_bg_image = junotoys_get_theme_option( 'front_page_testimonials_bg_image' );
...
<?php
	// Add anchor
	$junotoys_anchor_icon = junotoys_get_theme_option( 'front_page_testimonials_anchor_icon' );
	$junotoys_anchor_text = junotoys_get_theme_option( 'front_page_testimonials_anchor_text' );
...
?>
	...
	<?php
	if ( junotoys_get_theme_option( 'front_page_testimonials_fullheight' ) ) {
	...
	?>
			...
			<?php
			$junotoys_css      = '';
			$junotoys_bg_mask  = junotoys_get_theme_option( 'front_page_testimonials_bg_mask' );
			$junotoys_bg_color_type = junotoys_get_theme_option( 'front_page_testimonials_bg_color_type' );
			if ( 'custom' == $junotoys_bg_color_type ) {
				$junotoys_bg_color = junotoys_get_theme_option( 'front_page_testimonials_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_testimonials_content_wrap content_wrap">
			<?php
			// Caption
			$junotoys_caption = junotoys_get_theme_option( 'front_page_testimonials_caption' );
			if ( ! empty( $junotoys_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_testimonials_caption front_page_block_<?php echo ! empty( $junotoys_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $junotoys_caption, 'junotoys_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$junotoys_description = junotoys_get_theme_option( 'front_page_testimonials_description' );
			if ( ! empty( $junotoys_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_testimonials_description front_page_block_<?php echo ! empty( $junotoys_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $junotoys_description ), 'junotoys_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_testimonials_output">
				...
				if ( is_active_sidebar( 'front_page_testimonials_widgets' ) ) {
					...
					if ( ! junotoys_exists_trx_addons() ) {
						...
						junotoys_customizer_need_widgets_message( 'front_page_testimonials_caption', 'ThemeREX Addons - Testimonials' );
					}
				}
				?>
			</div>
