...
class Wpml
{
	...
	{
		if(!isset($_POST['wcmc_wpml_language']) || !$this->wpml_is_active())
			return;
		
		global $sitepress;
		load_plugin_textdomain('woocommerce-product-fields-checkout', false, WCMC_PLUGIN_LANG_PATH );
		$sitepress->switch_lang($_POST['wcmc_wpml_language'], true);
	}
	...
	public function switch_to_admin_default_lang()
	{
		...
		$this->before_admin_lang = $this->get_current_language_code();
	...
	public function switch_to_default_language()
	{
		if(!$this->wpml_is_active())
			return;
		global $sitepress;
		$this->curr_lang = $this->get_current_language_code() ;
	...
	function translate_single_string($id, $text, $prefix = "wcmc_label_"  )
	{
		if(!class_exists('SitePress'))
			return $text;
		
		$result = apply_filters( 'wpml_translate_single_string', $text, 'woocommerce-product-fields-checkout', 'wcmc_'.$prefix.$id, $this->get_current_language_code()  );
			
		return $result != "" ? $result : $text;
	}
	function register_single_string($id, $text, $prefix = "wcmc_label_" )
	{
		do_action( 'wpml_register_single_string', 'woocommerce-product-fields-checkout', 'wcmc_'.$prefix.$id, $text);
	}
	...
	public function get_current_locale()
	{
		global $sitepress;
													//en_US
		return !$this->wpml_is_active() ? get_locale() : $sitepress->get_locale($this->get_current_language_code());
	}
