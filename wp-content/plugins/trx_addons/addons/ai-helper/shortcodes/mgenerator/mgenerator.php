	...
	function trx_addons_sc_mgenerator( $atts, $content = '' ) {	
		...
		if ( ! Utils::is_music_api_available() ) {
			...
			trx_addons_get_template_part( array(
										TRX_ADDONS_PLUGIN_ADDONS . 'ai-helper/shortcodes/mgenerator/tpl.' . trx_addons_esc( $atts['type'] ) . '.php',
	...
	function trx_addons_sc_mgenerator_generate_music() {
		...
		if ( ! empty( $prompt ) ) {
			...
			if ( $api->get_api_key() != '' && ! $demo ) {
				...
				if ( ! empty( $_FILES["upload_music"]["tmp_name"] ) ) {
					$args['init_audio'] = trx_addons_uploads_save_data( trx_addons_fgc( $_FILES["upload_nusic"]["tmp_name"] ), array(
						'expire' => apply_filters( 'trx_addons_filter_ai_helper_uploaded_music_expire_time', 10 * 60 ),
					) );
				}
...
if ( ! function_exists( 'trx_addons_sc_mgenerator_is_prompt_not_english' ) ) {
	function trx_addons_sc_mgenerator_is_prompt_not_english( $prompt ) {
...
if ( ! function_exists( 'trx_addons_sc_mgenerator_translate_prompt' ) ) {
	...
	function trx_addons_sc_mgenerator_translate_prompt( $prompt, $args, $from = '' ) {
		...
		if ( in_array( $from, array( 'sc_mgenerator', 'media_library_generate_music' ) )
			...
			&& trx_addons_sc_mgenerator_is_prompt_not_english( $prompt )
		) {
...
if ( trx_addons_exists_gutenberg() && function_exists( 'trx_addons_gutenberg_get_param_id' ) ) {
	// require_once TRX_ADDONS_PLUGIN_DIR . TRX_ADDONS_PLUGIN_ADDONS . 'ai-helper/shortcodes/mgenerator/mgenerator-sc-gutenberg.php';
}
